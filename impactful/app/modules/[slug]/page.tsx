...
export default async function ModuleStartPage({ params }: { params: Promise<{ slug: string }> }) {
	...
	if (!moduleInfo) {
		return (
			<main className="min-h-screen bg-[#f3f1ec] px-4 py-8 text-black sm:flex sm:items-center sm:justify-center sm:px-6 sm:py-6">
				<section className="w-full max-w-[720px] rounded-[24px] bg-white px-6 py-8 shadow-[0_20px_80px_rgba(0,0,0,0.12)]">
					<p className="font-sans text-[12px] font-semibold uppercase tracking-[0.18em] text-[#98a0b3]">
						Module not found
					</p>
					<h1 className="mt-3 font-sans text-[32px] font-bold tracking-[-0.05em] text-black">
						That module is not available yet.
					</h1>
						<Link href="/modules" className="mt-8 inline-flex h-[48px] items-center justify-center bg-black px-6 font-mono text-[14px] font-bold tracking-[0.18em] text-white">
							[ BACK TO MODULES ]
					</Link>
				</section>
			</main>
	...
	return (
		<main className="min-h-screen bg-[#f3f1ec] px-4 py-8 text-black sm:flex sm:items-center sm:justify-center sm:px-6 sm:py-6">
			<section className="grid w-full max-w-[1040px] overflow-hidden bg-white shadow-[0_20px_80px_rgba(0,0,0,0.12)] lg:grid-cols-[1.05fr_0.95fr] lg:rounded-[28px]">
				<div className="bg-black px-6 py-8 text-white lg:px-10 lg:py-12">
					...
					<p className="mt-5 max-w-[340px] font-sans text-[17px] leading-[1.65] text-[#c7cfdd]">
				...
				<div className="px-6 py-8 lg:px-10 lg:py-12">
					...
					<h2 className="mt-3 font-sans text-[30px] font-bold leading-[1.05] tracking-[-0.05em] text-black">
						What happens next
					</h2>
					...
					<div className="mt-8 flex gap-4">
						<Link href="/modules" className="flex h-[52px] flex-1 items-center justify-center border-2 border-black bg-white font-mono text-[15px] font-bold tracking-[0.18em] text-black">
							[ BACK ]
						</Link>
						<Link href={`/modules/${slug}/tutorial`} className="flex h-[52px] flex-1 items-center justify-center bg-black font-mono text-[15px] font-bold tracking-[0.18em] text-white">
							[ CONTINUE ]
						</Link>
					</div>
				</div>
